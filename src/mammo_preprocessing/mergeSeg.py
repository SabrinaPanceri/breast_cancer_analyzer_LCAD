...
def crop_cancer(pathList, roiPathList, labelList, root, folder, sobrepos):
    ...
    while j < len(pathList):
        ...
        futureFile = roiPathList[j+1].split('_dataset/')
        auxi = futureFile[1].split('.')
        patientFutureFile = auxi[0]
        auxi2 = patientFutureFile.split('_')
        future = auxi2[0] + auxi2[1] + auxi2[2] + auxi2[3] + auxi2[4]
        auxi = patientFile.split('_')
        present = auxi[0] + auxi[1] + auxi[2] + auxi[3] + auxi[4]
        ...
        while present == future:
            ...
            tempImg = roiPathList[j+1] 
            imageBGR = cv2.imread(tempImg)
            image = cv2.cvtColor(imageBGR, cv2.COLOR_BGR2GRAY)
            tempImg2 = roiPathList[j] 
            imageBGR2 = cv2.imread(tempImg2)
            image2 = cv2.cvtColor(imageBGR2, cv2.COLOR_BGR2GRAY)

            matrixWithThePositionOfTheWhitePixels = np.where(image2 == [255])
            ...
            image[smallestWhitePixelOfCoordinateY:biggestWhitePixelOfCoordinateY, smallestWhitePixelOfCoordinateX:biggestWhitePixelOfCoordinateX]=image2
        ...
        if labelList[j] == True:
            ...
            for line in range(int(numberOfCroppedsY)): 
                ...
                for col in range(int(numberOfCroppedsX)):       
                    if np.where(image[temporarySmallestBlackPixelOfCoordinateY:temporaryBiggestBlackPixelOfCoordinateY, temporarySmallestBlackPixelOfCoordinateX:temporaryBiggestBlackPixelOfCoordinateX] == [255]) == True:
